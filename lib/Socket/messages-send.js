...
const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendAlbum: async (jid, mediaArray, options = {}) => {
            const validMedia = (mediaArray || []).filter(m => 'image' in m || 'video' in m);
            const userJid = authState.creds.me.id;
            const additionalAttributes = {};
            const albumMsg = await prepareAlbumMessageContent(jid, validMedia, {
                conn: {
                    relayMessage: relayMessage,
                    waUploadToServer: waUploadToServer,
                },
